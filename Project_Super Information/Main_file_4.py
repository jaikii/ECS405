...
#Author: Jai Kumar(15076) && Manav Mishra(15   )
"""Super Information"""
...
for file in files:
	...
	def Super_Entropy(dna):
		super_entropy=[]
		for j in range(12,202,3):			# Value for B
			N=j
	...
	xlabel("Super Entropy")
	ylabel("B(length of one cut of DNA)")
	plot([12+3*i for i in range(len(super_entropy_coding))],super_entropy_coding,color='red',label='coding')
	plot([12+3*i for i in range(len(super_entropy_coding))],super_entropy_non_coding,color='blue',label='non_coding')
	legend()
	savefig("%s.pdf"%file)
	show()
