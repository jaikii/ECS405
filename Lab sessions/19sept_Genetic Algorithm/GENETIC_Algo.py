...
for i in range(5):
	S=[]
	for j in range(1000): 
		c=[random.random() for l in range(2**(6+i))]
...
S=[]
for j in range(1000): 
	c=[random.random() for l in range(2**(6+2))]
	S1=[1 if k<1.0/3 else 0 for k in c]
	FT=abs(np.fft.fft(S1))
	s.append(FT[(len(FT)/3)]/mean(FT))
	S.append(S1)
def frac(s):
	t=0.0
	for i in range(len(s)):
...
while t1<0.3 and k<100:
	...
	while i <(len(S_)) and j<1000:
		...
		s.append(FT[(len(FT)/3)]*1.0/mean(FT))
		if random.random()<s[i]*1.0/4:				#selecting sequences out of 8000
			Snew.append(S_[i])
			j+=1
		i+=1
	print i
